...
function renderDescriptions(handlers) {
    return Promise.all(Object.entries(handlers).map(renderDescription)).then(ds => ds.join("\n\n"));
}

async function renderDescription([name, {title, prefixes, description, examples}]) {
    const renderedExamples = await Promise.all(examples.map((e, i) => renderExample(name, i + 1, e)));
    return `### ${title}\n\nView specification prefix${prefixes.length == 1 ? '' : 's'}: ${
...
function renderExample(handler, number, example) {
    ...
    if (example.screenshot === false)
        return preScreenshot;

    const filename = `example-${handler}-${number}.png`;
    return saveScreenshot(rawUrl, filename, example.viewport) 
        .then(path => `${preScreenshot}\n<a href="${rawUrl.replace(/&/g, "&amp;").replace(/"/g, "&quot;")}"><img alt="rendering of the URL shown above" src="${path}" style="border: 1px solid #ccc; max-height: 300px"/></a>\n`);
}
